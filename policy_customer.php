...
try {
    $query = "SELECT customer_policy, privacy_font, privacy_color, privacy_size FROM policy WHERE id = 1";
    ...
    die("Error fetching customer policy: " . $e->getMessage());
}
?>
        ...
        <div class="policy-container">
            ...
            <div class="mt-4">
                <p style="font-family: <?php echo htmlspecialchars($policy['privacy_font']); ?>; color: <?php echo htmlspecialchars($policy['privacy_color']); ?>; font-size: <?php echo (int)$policy['privacy_size']; ?>px;">
                    <?php echo nl2br(htmlspecialchars($policy['customer_policy'])); ?>
                </p>
            </div>
        </div>
    </div>
